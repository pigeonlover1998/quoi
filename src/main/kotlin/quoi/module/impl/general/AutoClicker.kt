...
//    private val clickInGui by BooleanSetting("Click while in inventory", desc = "Continues to auto click while the player is in inventory.")
    ...
    init {
        ...
        ac.sub("add") { button: String ->
            stupid(button) { list ->
                val uuid = player.mainHandItem.skyblockUuid ?: return@stupid ChatUtils.modMessage("&cYou are not holding a skyblock item!")
                if (list.contains(uuid)) return@stupid ChatUtils.modMessage("&cThis item is already in the $button list!")

                list.add(uuid)
                ChatUtils.modMessage("&aAdded ${player.mainHandItem.displayName.formattedString} &ato $button list!")
            }
        }.suggests("button", "left", "right")
        ac.sub("remove") { button: String ->
            stupid(button) { list ->
                val uuid = player.mainHandItem.skyblockUuid ?: return@stupid ChatUtils.modMessage("&cYou are not holding a skyblock item!")
                if (!list.remove(uuid)) return@stupid ChatUtils.modMessage("&cThis item is not in the $button list!")

                ChatUtils.modMessage("&aRemoved ${player.mainHandItem.displayName.formattedString} &afrom $button list!")
            }
        }.suggests("button", "left", "right")
        ac.sub("clear") { button: String ->
            stupid(button) { list ->
                list.clear()
                ChatUtils.modMessage("&aCleared the $button list!")
            }
        }.suggests("button", "left", "right")
    ...
    private fun startClicking(isLeft: Boolean) {
        if (isLeft) {
            ...
            leftJob = QuoiMod.scope.launch { click(true) }
        } else {
            if (rightJob != null) return
            rightJob = QuoiMod.scope.launch { click(false) }
        }
    }
    ...
    private suspend fun click(isLeft: Boolean) {
        ...
        while (true) {
            ...
            delay(if (min == max) min else Random.Default.nextLong(min, max))
        }
    }
    ...
    private fun stupid(button: String, block: (MutableList<String>) -> Unit) {
        val list = when (button.lowercase()) {
            "left" -> favLeft
            "right" -> favRight
            else -> return ChatUtils.modMessage("&cInvalid button! Use \"left\" or \"right\".")
        }
        block(list)
        Config.save()
    }
