...
//        }
        ...
        on<ChatEvent.Sent> {
            if (!chatEmotes || message.startsWith("/") && !ChatBypass.socialCommands.any { message.startsWith(it) }) return@on
            ...
            cancel()
            ChatUtils.say(words.joinToString(" "))
        }
    }
    ...
    private fun handleChatMessage(message: String): Boolean {
        ...
        if (cleanerDungeons) {
            for (r in toReplace) {
                ...
                ChatUtils.modMessage(m.value.replace(r.pattern, r.replacement), prefix = dungeonPrefix)
                return true
            }
        ...
        if (cleanerPf) {
            pfReplaceMap[noCodes]?.let { new ->
                ChatUtils.modMessage(new, prefix = "")
                return true
            }
            ...
            pfClassChangeRegex.find(noCodes)?.let { match ->
                ...
                ChatUtils.modMessage(cleanedMsg, prefix = "")
                return true
            }
        ...
        if (hideDiscordWarnings && discordWarningRegex.containsMatchIn(message)) {
            ...
            ChatUtils.modMessage(cleaned, prefix = "")
            return true
        }
