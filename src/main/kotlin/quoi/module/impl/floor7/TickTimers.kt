...
import quoi.utils.ThemeManager
...
object TickTimers : Module(
    ...
    private val padHud by textHud("Pad tick") {
        ...
        textSupplied(
            ...
            size = ThemeManager.theme.textSize,
            font = font,
            colour = colour
        ).shadow = shadow
    }.setting()
    ...
    private val deathTickHud by textHud("Death tick") { // maybe make an option to show it before dung start only
        visibleIf { Dungeon.deathTick >= 0 }
        textSupplied(
            supplier = { formatTime(if (preview) 15 else Dungeon.deathTick, 40) },
            size = ThemeManager.theme.textSize,
            font = font,
            colour = colour
        ).shadow = shadow
